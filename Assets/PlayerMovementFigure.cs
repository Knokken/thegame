...
public class PlayerMovementFigure : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (DialogueManagerCat2.dialogueEnded == false)
		{
			canDoRun = false;
			canDoWalk = false;
			canMove = false;
		...
		else if (DialogueManagerCat2.dialogueEnded == true && thisThing == false)
        {
			anim.SetBool("come", true);
			StartCoroutine(WaitForMoving());
			Debug.Log("Can move!");
		}
		else if (DialogueManagerCat2.dialogueEnded == true && thisThing == true)
        {
